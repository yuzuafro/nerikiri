...
| ブラシ演算 | `src/core/brushes.ts` | 7種ブラシ(押す/引く/なめらか/つまむ/ふくらます/ならす/塗る)の頂点変形・彩色、フォールオフ | F-03-*, F-04-01 |
...
- 初期形状: イコスフィア細分化レベル4(頂点2,562 / 三角形5,120)を基準とし、性能要件(NF-01)を満たす範囲で調整する。
...
- 各ブラシは `(mesh, hit, params) => 変更頂点集合` の共通インタフェース(NF-07)。
