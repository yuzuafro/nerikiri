...
export type ShapeKind = 'sphere' | 'manju' | 'tawara';
```
...
プリセット(F-01-03、v1.2で全面改訂)。いずれも底面 y=-1(作業台に接地):

| kind | 形状 | 変換(単位球の頂点 (x, y, z) に対して) |
|---|---|---|
| `sphere` | 球(丸めた直後の生地) | 恒等変換 |
| `nerikiri` | 練り切り基本形(上面が平ら・下すぼみ) | 上半球: `y' = (y - y³/3) × 0.83`(y=1 で勾配0 → 平らな上面)/ 下半球: `y' = y × 0.55`。水平: `(x,z) × 1.05 × (1 - 0.18·max(0,-y))`(下ほどすぼむ)。全体を `y' - 0.45` して底面を y=-1 に合わせる。高さ約1.10・幅約2.10 |
...
| 作業台 | PolarGridHelper 半径2.2・y=-1.05(粘土底面 y=-1 の直下)。正面・水平位置の基準(F-01-04) |
| 正面マーカー | ▲(ConeGeometry、橙 #ffb84d)を台の手前縁 (0, -1.03, 2.4) に配置、先端は粘土方向。初期カメラから見て手前=正面 |
